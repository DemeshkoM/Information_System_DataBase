 ...
 */
public class MedicationCategoryInsertController implements InsertController, Initializable {
    ...
    public void insertButtonTapped() {
        if (nameField.getText().isEmpty()) {
            ...
            if (insertMode == InsertMode.insert) {
                dbInit.insertMedicationCategory(name);
            } else {
                int id = DBInit.getIdFrom(item);
                dbInit.updateMedicationCategory(id, name);
            }
